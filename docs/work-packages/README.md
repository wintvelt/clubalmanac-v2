...
| WP12 | Data-migratie tooling (DynamoDB + S3 → Convex). Acht commando's in `scripts/migrate/` plus `convex/migration.ts`. Mechanisme = **API-import** (zelfgebouwde snapshot-zip getest en afgewezen: `invalid _id`). Vier audit-rondes, drie fix-cycli; twee productie-bugs vóór eerste gebruik gevangen — stil fotoverlies met een groene `verify`, en een `prune-storage` die de volledige T-2-upload kon wissen. A's spec-criticus vond zes semantische afwijkingen tussen bron en schema, waarvan `seenPics` óók fout in het migratieplan stond. Scope = tooling; de dev-seed en de prod-run staan nog open. | [`WP12-data-migratie-tooling.md`](./WP12-data-migratie-tooling.md) | ✅ tooling landed 2026-08-23 (dev-seed nog te draaien) |
...
| TBD | Video's van S3 naar Cloudflare R2. 10 clubfilms (~19 GB) + 9 cover-PNG's in bucket `blob-videos`, zonder DynamoDB-record — de oude app heeft de verwijzing hardcoded in de front-end. Buiten WP12 gehouden (besluit 2026-08-23): ~80% van de overzettijd voor nul feature-winst, plus een bestand van 5,8 GiB door een upload-URL zonder hervatting. Blijft op S3 tot dit WP draait; `blob-videos` mag daarom niet mee in de T+30-opruiming. | — | open |
