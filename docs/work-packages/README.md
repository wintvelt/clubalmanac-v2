...
| WP12 | Data-migratie tooling (DynamoDB + S3 → Convex). Acht commando's (`extract`/`inspect`/`transform`/`load-files`/`load-records`/`verify`/`prune-storage`/`reset`) in `scripts/`, één tool met een dev- en een prod-configuratie. Mechanisme = **API-import**: script schrijft via internal Convex-mutations, Convex mint `_id` en `_storage`. De zelfgebouwde-snapshot-zip-route is getest en afgewezen (`invalid _id`, 2026-08-23). Scope = tooling + dev-seed gedraaid; de prod-run staat in fase 5 op T-0. | [`WP12-data-migratie-tooling.md`](./WP12-data-migratie-tooling.md) | impl + 2 fix-cycli klaar, wacht op audit-ronde 3 |
