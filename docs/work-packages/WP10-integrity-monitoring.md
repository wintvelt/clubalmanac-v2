...
4. **Alert-dedup via state-tabel — strict-consecutive semantiek** **[audit-fix S-1]**: nieuwe `monitoringRuns`-tabel houdt per-run een `driftSignature` (hash van drift-rijen; leeg/`""` bij een schone run). Drift-email gaat uitsluitend wanneer de huidige run drift vindt **én** de huidige signature verschilt van die van de **direct voorgaande run** (de latest row op `by_runAt`), **ongeacht `emailSent`/`driftFound` van die vorige run**. Persistente identieke drift over opeenvolgende runs = één email, geen storm.

   > **Waarom strict-consecutive en niet "laatst-gealerte-run"** (audit S-1): de oude regel ("vergelijk met laatste run waarin `emailSent === true`") dedupt een **recurrence-na-clean** weg. Scenario: drift X → email; schone run (X opgelost) → geen email; drift X keert terug. Tegen de laatst-gealerte signature (nog steeds X) is X === X ⇒ géén re-alert — terwijl de terugkerende X conceptueel een **nieuw event** is dat de webmaster moet zien. Door tegen de *direct voorgaande* run te vergelijken reset de schone run de referentie naar `""`, zodat de terugkerende X (X ≠ `""`) wél opnieuw alarmeert. De prijs — een drift die één run lang "flikkert" en in de volgende identiek terugkomt gaat als nieuw event door — is gewenst: flikker-recurrence is precies wat de webmaster wil weten, en op een 16-user-app met een daily cron is alert-storm geen reëel risico.
...
6. **Maandelijkse heartbeat-email**: één keer per 30 dagen stuurt monitor een "alive"-email naar `WEBMASTER_EMAILS`, ongeacht drift-status. Passive meta-monitoring zodat afwezigheid-van-email niet automatisch als "geen drift" wordt geïnterpreteerd. Implementatie (A's keuze): inline state-check binnen dezelfde monitor-mutation ("als 30+ dagen sinds laatste mail én deze run queue't géén drift-mail, stuur heartbeat").

   **Heartbeat-tekst mag niet liegen bij gededupte drift [audit-fix N-1]**: een heartbeat kan vuren op een run waarin er **wél** drift is, maar die door de strict-consecutive dedup (invariant 4) geen drift-mail stuurt — bijv. persistente identieke drift die 30+ dagen onveranderd blijft. De huidige tekst ("Geen nieuwe drift sinds de vorige melding") is dan **misleidend**: ze suggereert all-clear terwijl er openstaande drift is. Daarom is de heartbeat-tekst **conditioneel** op of de run drift vond:
   - **Schone run** (geen drift): "alive"-bevestiging + expliciet géén drift gedetecteerd.
   - **Gededupte-drift-run** (drift aanwezig, maar identiek aan vorige run): "alive"-bevestiging **plus** een waarschuwende zinsnede dat er **openstaande drift** is die sinds een eerdere melding onveranderd aanwezig blijft, met verwijzing naar het Convex **dashboard** voor details. De heartbeat herhaalt de drift-IDs niet (PII-grens + één-email-discipline blijven gelden) — het dashboard-log is autoritatief (invariant 5).

   Implicatie voor B: de heartbeat-action moet weten of de huidige run drift vond (`driftFound` doorgeven), en `monitorHeartbeatTemplate` krijgt een `driftPresent`-parameter die de conditionele zinsnede schakelt. Verplichte marker-zinsnede in de drift-aanwezig-tak (zodat de test ondubbelzinnig kan pinnen): bevat de tekst **"openstaande drift"** en **"dashboard"**. De schone tak bevat deze marker niet.
