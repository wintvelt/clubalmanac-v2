...
- **WP6 deferred naar pre-cutover** (uit WP6-spec audit-follow-up): integration-test `tests/integration/clerk/onboardingWebhook.test.ts` + drie empirische mens-gates — Gate 1 happy-path signup-completion, Gate 2 idempotency re-login, Gate 3 zero-invite fallback. Wouter draait alle drie handmatig vóór cutover.
