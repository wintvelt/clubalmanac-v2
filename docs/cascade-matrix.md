...
- **U10 cleanup-keuze:** alleen `flaggedBy` op `undefined`, flag-state blijft op photo. Implementatie loopt via `by_flagged` index (sparse op flaggedAt) gefilterd op `flaggedBy === user._id`. Aparte `by_flagger` index niet toegevoegd: bij 20 users + bescheiden flag-volume is full-scan over flagged-only acceptabel
